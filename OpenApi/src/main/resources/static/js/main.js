...
const getAirQuality = async (cityName) => {
  ...
  /*
    async : 비동기 요청 코드가 포함된 함수에 작성하는 키워드
      ...
      - async가 포함된 함수에서
        명시적으로 Promise 객체 반환 구문을 약속하지 않을 경우
  ...
  */
  ...
  //   응답 결과(첫 번째 then의 resopnse)가 저장됨
  ...
  try {
    ...
    // 이모지/등급 배열
    const gradeEmoji = ['a','😄', '🙂', '😷', '🤢'];
    const gradeText = ['a','좋음', '보통', '나쁨', '매우나쁨'];

    pm10Grade.innerText = gradeEmoji [item.pm10Grade];
    pm10GradeText.innerText = gradeText [item.pm10Grade];
    pm10Value.innerText     = `미세먼지 농도 : ${item['pm10Value']} ㎍/㎥`;

    pm25Grade.innerText = gradeEmoji [item.pm25Grade ];
    pm25GradeText.innerText = gradeText [item.pm25Grade];
    pm25Value.innerText     = `미세먼지 농도 : ${item['pm25Value']} ㎍/㎥`;
